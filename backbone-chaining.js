  ...
  (function(factory) {
    ...
    Chaining = Backbone.Chaining = {
      ...
      Events: {
        ...
        _splitEvents: function(events, callback, context) {
          var chainEvents, obj1, primitives;
          if (_.isString(events)) {
            events = (
              obj1 = {},
              obj1["" + events] = callback,
              obj1
            );
          } else {
            if (context == null) {
          ...
          chainEvents = [];
          primitives = {};
          _.each(events, function(callback, names) {
            var primitiveNames;
            primitiveNames = [];
            _.each(names.split(/\s+/), function(name) {
              var chainEvent;
              if (chainEvent = Chaining.parseEvent(name)) {
                return chainEvents.push(_.extend(chainEvent, {
                  callback: callback
                }));
              } else {
                return primitiveNames.push(name);
              }
            });
            if (primitiveNames.length > 0) {
              return primitives[primitiveNames.join(' ')] = callback;
            }
          });
          return [chainEvents, primitives, context];
        },
        _addEventChainProxy: function(event, context) {
          if (this._eventChainProxies == null) {
            this._eventChainProxies = [];
          }
          return this._eventChainProxies.push(new Chaining.EventChainProxy(this, event.name, event.attr, event.callback, context));
        },
        ...
        _setupChaining: function(names, callback, context, chainedCB) {
          var chainEvents, primitives, ref;
          ref = this._splitEvents(names, callback, context), chainEvents = ref[0], primitives = ref[1], context = ref[2];
          _.each(chainEvents, (function(_this) {
            return function(event) {
              return _this._addEventChainProxy(event, context);
            };
          })(this));
        ...
        on: function(names, callback, context) {
          var primitives, ref;
          ref = this._setupChaining(names, callback, context), primitives = ref[0], context = ref[1];
          return this._primitiveOn(primitives, context);
        },
        listenTo: function(obj, names, callback) {
          var context, primitives, ref;
          ref = obj._setupChaining(names, callback, this, (function(_this) {
            return function() {
              return _this._primitiveListenTo(obj, _this._dummyEvent, function() {});
            };
          })(this)), primitives = ref[0], context = ref[1];
          return this._primitiveListenTo(obj, primitives);
        },
        off: function(names, callback, context) {
          var chainEvents, primitives, ref, ret;
          if (!names) {
            this._removeEventChainProxy(null, callback, context);
            return this._primitiveOff(names, callback, context);
          }
          ref = this._splitEvents(names, callback, context), chainEvents = ref[0], primitives = ref[1], context = ref[2];
          _.each(chainEvents, (function(_this) {
            return function(event) {
              return _this._removeEventChainProxy(event, event.callback, context);
            };
          })(this));
          ret = void 0;
          _.each(primitives, (function(_this) {
            return function(callback, name) {
              return ret = _this._primitiveOff(name, callback, context);
            };
          })(this));
          return ret;
        }
    ...
    Backbone.Chaining.EventChainProxy = (function() {
      ...
      function EventChainProxy(requester1, eventName1, attr1, callback1, context1) {
      ...
      EventChainProxy.prototype.listenToRemotes = function() {
        ...
        if (_.isFunction(this.requester.get)) {
          ...
          this.updateEvent = "change:" + (((ref = Chaining.parseChain(this.attr)) != null ? ref.head : void 0) || this.attr);
